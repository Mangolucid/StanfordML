...
regfactor = (lambda/(2*m))*( sum(sum(Theta1(:, 2:end).^2)) + sum(sum(Theta2(:, 2:end).^2)) );

J = J + regfactor;

delta3 = a3 .- y_vec; %5000x10

delta2 = delta3*Theta2.*sigmoidGradient([zeros(size(z2, 1), 1) z2]); %Critical line of code
